...
{
    /// <summary>
    /// 수갑 적재 칸. PrisonerQueueManager 가 소유한 buffer 와 head 죄수 sink 를 Init 으로 받음.
    /// holder 타입별 동작:
    ///   Player (IsPlayerControlled): full logic — sink ready 면 hand/buffer→sink, 미ready 면 hand→buffer
    ///   Worker (IsPlayerControlled=false): hand→buffer 만 (deposit 역할 전용)
    /// stack 시각화는 자기 transform 에서 표시 (이 zone 이 buffer 의 시각 위치).
    /// 모든 transfer 는 ItemFlowManager.Fly 로 시각화 — source 즉시 차감 + 도착 시 destination accept.
    /// </summary>
    [RequireComponent(typeof(BoxCollider))]
    [RequireComponent(typeof(Rigidbody))]
    public class HandcuffInputZone : MonoBehaviour
    {
        [Header("Resource")]
        ...
        [SerializeField] private Transform bufferStackAnchor;
        [SerializeField] private Vector3 bufferStackOffsetStep = new Vector3(0f, 0.18f, 0f);

        public Transform BufferStackAnchor => bufferStackAnchor != null ? bufferStackAnchor : transform;
        ...
        public void Init(StockpileModel _sharedBufferStockpile, IResourceSink _downstreamSink)
        {
            ...
            {
                Debug.LogError(
                    $"[HandcuffInputZone] {name}: 자원 타입 불일치 — zone={expectedResourceType}, sink={_downstreamSink.InputType}.");
                downstreamSink = null;
            }
            else
            {
            ...
            // buffer 시각화는 Init 에서 (Init 시점에 buffer 가 확정됨).
            // anchor 가 별도 지정되어 있으면 거기에 쌓이고, 없으면 zone transform 자체를 사용.
            if (sharedBufferStockpile != null && bufferStockVisualizer == null)
            {
                SystemManager systemManager = SystemManager.Instance;
                PoolManager pool = systemManager != null ? systemManager.Pool : null;
                bufferStockVisualizer = new StackVisualizer(
                    sharedBufferStockpile.Count,
                    BufferStackAnchor,
        ...
        private void Update()
        {
            ...
            // 1 transfer per tick. 첫 성공 시 return.
        ...
        private bool TryPlayerFullLogic(IInventoryHolder _holder)
        {
            ...
            if (sinkReady && handCount == 0 && sharedBufferStockpile != null && sharedBufferStockpile.HasStock)
            {
                ...
                {
                    downstreamSink.TryAcceptOne();
                    LaunchFlightDecoration(BufferStackAnchor.position, GetSinkPosition(), ResolveSinkArrivalCallback());
                    return true;
                }
            }
            ...
            if (!sinkReady && handCount > 0 && sharedBufferStockpile != null && sharedBufferStockpile.HasSpace)
            {
                if (_holder.Inventory.TryRemove(expectedResourceType, 1))
                {
                    ...
                    LaunchFlightDecoration(GetHolderPosition(_holder), BufferStackAnchor.position, null);
                    return true;
                }
            }
        ...
        private bool TryDepositOnly(IInventoryHolder _holder)
        {
            // Worker: hand → buffer 만
            if (sharedBufferStockpile == null || !sharedBufferStockpile.HasSpace) return false;
            if (_holder.Inventory.GetCount(expectedResourceType) <= 0) return false;
